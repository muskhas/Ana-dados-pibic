...
UsoDeAgua=data.frame(usaswartzia, usagoupia, usascleronema, usaeriotheca, usabuchenavia, usaocotea, usaeschweilera,time= temp_jj$tempo)
...
```{r}
colnames(UsoDeAgua)
UsoDeAgua$data=as.Date(UsoDeAgua$time) #agrupando os dados dos dias em um só 

Uda_dia =UsoDeAgua%>%group_by(data) %>% summarise(sp1=sum(usaswartzia,na.rm=T), sp2 = sum(usagoupia,na.rm=T), sp3= sum(usascleronema,na.rm=T), sp4 = sum(usaeriotheca,na.rm=T), sp5 = sum(usabuchenavia,na.rm=T), sp6 = sum(usaocotea,na.rm=T), sp7 = sum(usaeschweilera,na.rm=T)) #uso de água por dia

View(Uda_dia)
##Fazer os gráficos para cada dia
ggplot(Uda_dia,aes(x=data,y=sp1)) + geom_point() +ylab("Eschweilera") + xlab("Tempo") + ggtitle("Uso de água/dia")

#Média das variáveis ambientais por dia e fazer os gráficos
```
